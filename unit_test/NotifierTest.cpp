...
TEST ( Notifier, TestOfNotification )
{
	...
	notifier.addNotification(getNotificationTest1(),Utils::makeCallback(&testClazz,&TestClazz::onDeliverNotification));
	...
	notifier.addNotification(getNotificationTest1(),Utils::makeCallback(&testClazz,&TestClazz::onDeliverNotification));
	notifier.addNotification(getNotificationTest2(),Utils::makeCallback(&testClazz,&TestClazz::onDeliverNotification));
...
TEST ( Notifier, SpeedTestNotify )
{
	...
	CCNotificationCenter* pCenter = CCNotificationCenter::sharedNotificationCenter();
	...
	delete pTab;
	{
		...
		pNotifier->addNotification(getNotificationTest4(),Utils::makeCallback(pTestClazz,&TestClazz::onDeliverNotification2));
		pCenter->addObserver(pTestClazz,callfuncO_selector(TestClazz::onOldNotificationFloat),OLD_NOTIFICATION_4,nullptr);
...
TEST ( Notifier, SpeedTestNotifyMultipleListeners )
{
	...
	for(int i=0;i<listenersCount;++i)
	{
		...
		pNotifier->addNotification(getNotificationTest4(),Utils::makeCallback(listeners.back(),&TestClazz::onDeliverNotification2));

		pCenter->addObserver(listeners.back(),callfuncO_selector(TestClazz::onOldNotificationFloat),OLD_NOTIFICATION_4,nullptr);
...
TEST ( Notifier, SpeedTestNotifyMultipleListenersMultipleNotifications )
{
	...
	for(int i=0;i<listenersCount;++i)
	{
		...
		if(which == 0)
		{
			pNotifier->addNotification(getNotificationTest1(),Utils::makeCallback(listeners.back(),&TestClazz::onDeliverNotification));
			pCenter->addObserver(listeners.back(),callfuncO_selector(TestClazz::onOldNotification),OLD_NOTIFICATION_1,nullptr);
		}
		else if( which== 1)
		{
			pNotifier->addNotification(getNotificationTest2(),Utils::makeCallback(listeners.back(),&TestClazz::onDeliverNotification));
			pCenter->addObserver(listeners.back(),callfuncO_selector(TestClazz::onOldNotification),OLD_NOTIFICATION_2,nullptr);
		}
		else if( which== 2)
		{
			pNotifier->addNotification(getNotificationTest3(),Utils::makeCallback(listeners.back(),&TestClazz::onDeliverNotification3));
			pCenter->addObserver(listeners.back(),callfuncO_selector(TestClazz::onOldNotificationInt),OLD_NOTIFICATION_3,nullptr);
		}
		else if( which== 3)
		{
			pNotifier->addNotification(getNotificationTest4(),Utils::makeCallback(listeners.back(),&TestClazz::onDeliverNotification2));
